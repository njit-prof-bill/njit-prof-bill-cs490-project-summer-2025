...
interface JobAdRecord {
  ...
  export async function GET(request: Request) {
    ...
    const payload = records.map((r) => ({
      id: r._id.toString(),
      url: r.url,
      rawText: r.rawText,                // ← include rawText here
  ...
  export async function POST(request: Request) {
    ...
    if (!body.url && !body.rawText) {
      return NextResponse.json({ error: "Provide a URL or rawText" }, { status: 400 });
    }
  ...
  `.trim();
    ...
    const record: JobAdRecord = {
      userId: decoded.uid,
      url: body.url,
      rawText: body.rawText,           // ← save rawText too
    ...
    return NextResponse.json({
      ...
      url: body.url,
      rawText: body.rawText,           // ← echo rawText back
      companyName: parsed.companyName,
      jobTitle: parsed.jobTitle,
