...
const EducationEditor: React.FC<EducationEditorProps> = ({ onSuccess, onError }) => {
  ...
  const handleNewEducationChange = React.useCallback((field: keyof Education, value: string) => {
    setNewEducation(prev => ({ ...prev, [field]: value }));
  }, []);

  const handleEditingEducationChange = React.useCallback((field: keyof Education, value: string) => {
  ...
  // Move EducationForm outside of render to prevent re-creation on each render
  const EducationForm = React.useCallback(({ 
    educationData, 
    onChange, 
    onSave, 
    onCancel, 
