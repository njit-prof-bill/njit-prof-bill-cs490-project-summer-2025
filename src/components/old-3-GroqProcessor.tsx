    ...
    try {
      ...
      for (const mapping of documentMappings) {
        ...
        if (targetDoc.exists()) {
          ...
          if (docData.isValidJSON && docData.groqResponse) {
            const validation = validateJSON(docData.groqResponse);
            if (validation.isValid && validation.parsed) {
              processedData.push(validation.parsed);
      ...
      if (processedData.length === 0) {
        setCombineStatus('❌ No valid processed documents found to combine');
        return;
      }

      setCombineStatus(`Combining ${processedData.length} documents with AI...`);

      // Prepare the combine prompt with all valid JSON data
      const jsonStrings = processedData.map(data => JSON.stringify(data, null, 2));
      ...
      if (!combinedResponse) {
        setCombineStatus('❌ Failed to combine documents with AI');
        return;
      }
      ...
      await setDoc(finalDocRef, {
        ...
        numberOfSourceDocuments: processedData.length,
        sourceDocumentNames: documentNames
      });

      setCombineStatus(`✅ Successfully combined ${processedData.length} documents into final result`);
    } catch (error) {
      console.error('Error combining documents:', error);
      setCombineStatus('❌ Error combining documents');
    } finally {
